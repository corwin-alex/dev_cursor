...
- Перезапустите сервис и проверьте, что данные очистились (ожидаемо для демо-режима).

## Рекомендации для Render

- Использовать один сервис (Express отдает и API, и собранный фронтенд).
- Для бесплатного инстанса и демо использовать `DATABASE_PATH=:memory:`.
- Учитывать, что данные SQLite будут сбрасываться при рестарте инстанса.
- Если позже понадобится сохранение данных, переходите на Persistent Disk или внешнюю БД.
